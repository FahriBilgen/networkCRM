"""Integration test for a full 30-turn run with real Ollama models."""
...
def test_full_run_30_turns(tmp_path: Path) -> None:
    """Run 30 turns with real Ollama models, validate outputs and artifacts."""
    if os.environ.get("FORTRESS_USE_OLLAMA") != "1":
        pytest.skip("Skipping real model test; set FORTRESS_USE_OLLAMA=1 to run")

    out_dir = tmp_path / "artifacts" / "full_run_30"
    ...
    for turn in range(1, 31):
        ...
        if result.get("win_loss", {}).get("status") in ["loss", "win"]:
            ...
            assert (
                isinstance(result["options"], list) and 1 <= len(result["options"]) <= 3
            )
    ...
    for opt in result["options"]:
        assert "id" in opt and "text" in opt and "action_type" in opt
        assert opt["text"].strip()
    assert isinstance(result["character_reactions"], list)
    metrics = result["metrics_after"]
    keys = ["glitch", "order", "morale", "resources", "knowledge", "corruption"]
    assert all(k in metrics for k in keys)
    ...
    if any("change_weather" in str(effect) for effect in result.get("effects", [])):
        safe_functions_executed = True
        assert state.get("world_constraint") != baseline.get("world_constraint")

        win_loss = result["win_loss"]
        if win_loss.get("consistent", False):
            judge_consistent_count += 1
        assert "reason" in win_loss and win_loss["reason"].strip()

        win_loss_values.append(win_loss.get("status", "ongoing"))

        with open(out_dir / f"turn_{turn:02d}.json", "w", encoding="utf-8") as f:
            json.dump(result, f, ensure_ascii=False, indent=2)
        with open(out_dir / f"state_{turn:02d}.json", "w", encoding="utf-8") as f:
    ...
    assert judge_consistent_count >= 27
